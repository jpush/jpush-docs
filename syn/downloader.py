...
def git_init():
    if (not os.path.exists("/opt/push/jpush-docs/syndocs/")):
        logging.info(os.mkdir("/opt/push/jpush-docs/syndocs/"))

    if (not os.path.exists("/opt/push/jpush-docs/syndocs/autosyn.log")):
        logging.info(commands.getstatusoutput("touch /opt/push/jpush-docs/syndocs/autosyn.log"))

    if (not os.path.exists("/opt/push/jpush-docs/syndocs/jpush-docs/")):
        logging.info(commands.getstatusoutput("git clone git@github.com:jpush/jpush-docs.git"))
...
def git_push():
    ...
    push_result= (commands.getstatusoutput("git push origin master"))
    ...
    if(push_result[0]):
        ...
        print ("success")
        logging.info("git push origin master")
