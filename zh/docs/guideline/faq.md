...
可通过调用代码 [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"prefs:root=NOTIFICATIONS_ID&&path=当前应用的bundleid"]] 进入您的应用的通知设置页面，引导用户手动变更“允许通知”的状态。
...
+ 如果是v3 api推送，请确认是否使用了apns_production参数，值是否为：true。
