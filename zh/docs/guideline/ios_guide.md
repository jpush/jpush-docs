...
* APS_FOR_PRODUCTION 
    * 1.3.1版本新增，表示应用是否采用生产证书发布( Ad_Hoc 或 APP Store )，此参数值需要与应用（code signing）所使用的证书环境一致，0 (默认值)表示采用的是开发者证书，1 表示采用生产证书发布应用。同时服务端推送消息时，需要设置Option中apns_production为对应证书的环境，若不设置该参数，服务端SDK默认推送到开发环境。
