...
content-available| boolean| 可选| 推送唤醒| 推送消息携带 content-available: true 是Background 运行的必须参数，如果不携带此字段则与iOS7 之前版本的普通推送一样。
...
```	   
 ...
 JPush 因为需要重新组包，并且考虑一点安全冗余，要求本部分内容总体长度限制为：220 个字节。
...
apns_production|	boolean|	可选|	APNs是否生产环境|	True 表示推送生产环境，False 表示要推送开发环境。JPush 官方 API LIbrary (SDK) 默认设置为推送 “开发环境”。
...
Code |	描述 |	详细解释 |	实际提示信息 |	HTTP Status Code
:---| :---| :---| :---| :---
